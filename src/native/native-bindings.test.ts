...
const BENCHMARK_FIXTURE = "test/benchmarks/openapi.localhost4000.json";
const benchmarkTest = existsSync(BENCHMARK_FIXTURE) ? test : test.skip;
...
describe("native OpenAPI bindings", () => {
  ...
  test("renders explicit types for mutually recursive schemas", () => {
    ...
    expect(models).not.toContain("z.ZodType<Player> = z.compile(");
    expect(models).not.toContain("z.ZodType<AccessTag> = z.compile(");
  });
  ...
  test("uses the module tag for module-local model namespaces", () => {
    ...
    expect(actual.renderedModels.Chat).toContain("export const MessageSchema = z.compile(z.object(");
    expect(actual.renderedModels.Chat).not.toContain("export namespace CommonModels {");
  });

  benchmarkTest("matches TypeScript operation names and component ownership", async () => {
    const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
  ...
  benchmarkTest.each([true, false])(
    "matches all TypeScript component schemas with extractEnums=%s",
    async (extractEnums) => {
      const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
      const document = JSON.parse(source);
      const options = resolveConfig({
  ...
  benchmarkTest("matches the complete TypeScript endpoint and schema data", async () => {
    const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
    const document = JSON.parse(source);
    const options = resolveConfig({ fileConfig: { input: "fixture", output: "output" }, params: {} });
  ...
  benchmarkTest("matches TypeScript models-in-common proxy rendering", async () => {
    const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
    const document = JSON.parse(source);
    const options = resolveConfig({
  ...
  benchmarkTest("matches TypeScript models-in-common endpoint rendering", async () => {
    const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
    const document = JSON.parse(source);
    const options = resolveConfig({
      fileConfig: {
  ...
  benchmarkTest.each([false, true, { exclude: ["Currencies/updateCurrency"] }])(
  ...
  benchmarkTest("matches TypeScript ACL module rendering", async () => {
    const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
    const document = JSON.parse(source);
    const options = resolveConfig({
  ...
  benchmarkTest("matches TypeScript shared-file rendering", async () => {
    const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
    const document = JSON.parse(source);
    const options = resolveConfig({
      fileConfig: { input: "fixture", output: "output", modelsInCommon: true, workspaceContext: [] },
  ...
  benchmarkTest("fully renders namespace-free module-local output with TypeScript parity", async () => {
    const source = await fs.readFile(BENCHMARK_FIXTURE, "utf8");
    const document = JSON.parse(source);
    const options = resolveConfig({
      fileConfig: {
