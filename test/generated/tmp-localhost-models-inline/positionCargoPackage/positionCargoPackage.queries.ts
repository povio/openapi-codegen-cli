...
const createPackage = (officeId: string, positionId: string, cargoId: string, data: CommonModels.CreatePositionCargoPackageDTO) => {
...
const updatePackage = (officeId: string, positionId: string, cargoId: string, packageId: string, data: CommonModels.UpdatePositionCargoPackageDTO) => {
...
const movePackage = (officeId: string, positionId: string, cargoId: string, packageId: string, data: CommonModels.MovePositionCargoPackageRequestDTO) => {
  return AppRestClient.patch(
    { resSchema: CommonModels.PositionCargoPackageResponseDTOSchema },
 ...
 * @statusCodes [201, 401]
 */
export const useCreatePackage = (options?: AppMutationOptions<typeof createPackage, { officeId: string, positionId: string, cargoId: string, data: CommonModels.CreatePositionCargoPackageDTO }> & MutationEffectsOptions) => {
 ...
 * @statusCodes [200, 401]
 */
export const useUpdatePackage = (options?: AppMutationOptions<typeof updatePackage, { officeId: string, positionId: string, cargoId: string, packageId: string, data: CommonModels.UpdatePositionCargoPackageDTO }> & MutationEffectsOptions) => {
  const queryConfig = OpenApiQueryConfig.useConfig();
  const { checkAcl } = useAclCheck();
  const { runMutationEffects } = useMutationEffects({ currentModule: moduleName });
 ...
 * @statusCodes [200, 401]
 */
export const useMovePackage = (options?: AppMutationOptions<typeof movePackage, { officeId: string, positionId: string, cargoId: string, packageId: string, data: CommonModels.MovePositionCargoPackageRequestDTO }> & MutationEffectsOptions) => {
  const queryConfig = OpenApiQueryConfig.useConfig();
  const { checkAcl } = useAclCheck();
  const { runMutationEffects } = useMutationEffects({ currentModule: moduleName });
