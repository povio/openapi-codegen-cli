...
export namespace ChargeTypesApi {
      ...
      `/charge-types/paginate/labels`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(ChargeTypesModels.ChargeTypesFindAllOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(ChargeTypesModels.ChargeTypeLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
  ...
  export const paginate = (
    ...
    return AppRestClient.get({ resSchema: ChargeTypesModels.ChargeTypesPaginateResponseSchema }, `/charge-types`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(ChargeTypesModels.ChargeTypesPaginateOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(ChargeTypesModels.ChargeTypePaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
        limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const update = (
    id: string,
    data: ChargeTypesModels.UpdateChargeTypeRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
