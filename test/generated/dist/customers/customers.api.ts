...
export namespace CustomersApi {
  export const findProfile = (config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: CustomersModels.CustomerProfileResponseDTOSchema }, `/customers/me`, config);
  };
  ...
  export const list = (
    ...
    return AppRestClient.get({ resSchema: CustomersModels.CustomersListResponseSchema }, `/customers`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(CustomersModels.CustomersListOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(CustomersModels.CustomerPaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
