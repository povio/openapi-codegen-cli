...
export namespace ControlTowerBookingsQueries {
  ...
  export const keys = {
    ...
    findAll: (
      ...
      page?: number,
      cursor?: string,
    ) => [...keys.all, "/bookings", limit, order, filter, page, cursor] as const,
    findAllInfinite: (
      limit?: number,
      order?: string,
      filter?: ControlTowerBookingsModels.BookingFilterDto,
      cursor?: string,
    ) => [...keys.all, "/bookings", "infinite", limit, order, filter, cursor] as const,
    findById: (id: string) => [...keys.all, "/bookings/:id", id] as const,
    findPackageById: (packageId: string, bookingId: string) =>
      [...keys.all, "/bookings/:bookingId/packages/:packageId", packageId, bookingId] as const,
  };
  ...
  export const useFindAll = <TData>(
    ...
    options?: AppQueryOptions<typeof ControlTowerBookingsApi.findAll, TData>,
    config?: AxiosRequestConfig,
  ) => {
    return useQuery({
      queryKey: keys.findAll(limit, order, filter, page, cursor),
      queryFn: () => ControlTowerBookingsApi.findAll(limit, order, filter, page, cursor, config),
      ...options,
    });
  };
  ...
  export const useFindAllInfinite = <TData>(
    {
      ...
      filter,
      cursor,
    }: { limit: number; order?: string; filter?: ControlTowerBookingsModels.BookingFilterDto; cursor?: string },
    options?: AppInfiniteQueryOptions<typeof ControlTowerBookingsApi.findAll, TData>,
    config?: AxiosRequestConfig,
  ) => {
    return useInfiniteQuery({
      queryKey: keys.findAllInfinite(limit, order, filter, cursor),
      queryFn: ({ pageParam }) => ControlTowerBookingsApi.findAll(limit, order, filter, pageParam, cursor, config),
      initialPageParam: 1,
  ...
  export const useFindById = <TData>(
    { id }: { id: string },
    options?: AppQueryOptions<typeof ControlTowerBookingsApi.findById, TData>,
    config?: AxiosRequestConfig,
  ) => {
    return useQuery({
      queryKey: keys.findById(id),
      queryFn: () => ControlTowerBookingsApi.findById(id, config),
      ...options,
    });
  };
   ...
   * @statusCodes [200, 401]
   */
  export const useFindPackageById = <TData>(
    { packageId, bookingId }: { packageId: string; bookingId: string },
    options?: AppQueryOptions<typeof ControlTowerBookingsApi.findPackageById, TData>,
    config?: AxiosRequestConfig,
  ) => {
    return useQuery({
      queryKey: keys.findPackageById(packageId, bookingId),
      queryFn: () => ControlTowerBookingsApi.findPackageById(packageId, bookingId, config),
      ...options,
    });
  };
}
