...
export namespace MasterDataImportModels {
   ...
   */
  export const MasterDataImportRequestDtoSchema = z
    .object({
      mediaId: z.string().describe("Media ID of the uploaded file"),
      type: ImportTypeEnumSchema.describe("Type of data to import"),
    })
    .readonly();
   ...
   */
  export const ImportResultDtoSchema = z
    .object({
      importStatus: ImportStatusEnumSchema.describe("Import result status").nullish(),
      downloadUrl: z.string().describe("S3 presigned URL for result file download"),
      expiresAt: z.iso.datetime({ offset: true }).describe("Download URL expiration time").nullish(),
   ...
   */
  export const ImportStatusResponseDtoSchema = z
    .object({
      status: z.string().describe("Current job status").nullable(),
      result: ImportResultDtoSchema.describe("Import result data when job is completed").nullish(),
    })
    .readonly();
   ...
   */
  export const MasterDataImportUploadRequestDtoSchema = z
    .object({
   ...
   */
  export const MasterDataImportUploadResponseDtoSchema = z
    .object({
      mediaId: z.string().describe("Media ID for the uploaded file"),
      url: z.string().describe("S3 presigned upload URL"),
    })
    .readonly();
   ...
   */
  export const MasterDataImportResponseDtoSchema = z
    .object({
      jobId: z.string().describe("PG Boss job ID"),
      status: z.string().describe("Initial job status").nullish(),
    })
    .readonly();
