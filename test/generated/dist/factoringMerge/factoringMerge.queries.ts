...
export namespace FactoringMergeQueries {
  ...
  export const keys = {
    ...
    getMergeBatch: (officeId: string, batchId: string) =>
      [...keys.all, "/offices/:officeId/factoring-merge/:batchId", officeId, batchId] as const,
  };
  ...
  export const usePrepareUpload = (
    options?: AppMutationOptions<
      typeof FactoringMergeApi.prepareUpload,
      { officeId: string; data: FactoringMergeModels.PrepareFactoringMergeRequestDto }
    > &
   ...
   * @statusCodes [201, 401, default]
   */
  export const useProcessMerge = (
    options?: AppMutationOptions<typeof FactoringMergeApi.processMerge, { officeId: string; batchId: string }> &
      MutationEffectsOptions,
    config?: AxiosRequestConfig,
  ) => {
    const { checkAcl } = useAclCheck();
   ...
   */
  export const useGetMergeBatch = <TData>(
    { officeId, batchId }: { officeId: string; batchId: string },
    options?: AppQueryOptions<typeof FactoringMergeApi.getMergeBatch, TData>,
    config?: AxiosRequestConfig,
  ) => {
    const { checkAcl } = useAclCheck();
