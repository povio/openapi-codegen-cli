...
export namespace ChecklistTemplatesApi {
  export const create = (
    officeId: string,
    data: ChecklistTemplatesModels.CreateChecklistTemplateRequestDTO,
      ...
      `/offices/${officeId}/checklist-templates`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(ChecklistTemplatesModels.ChecklistTemplatesPaginateOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(ChecklistTemplatesModels.ChecklistTemplateFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
    ...
    return AppRestClient.get(
      ...
      `/offices/${officeId}/checklist-templates/paginate/labels`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(
              ChecklistTemplatesModels.ChecklistTemplatesPaginateLabelsOrderParamEnumSchema,
            ).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(ChecklistTemplatesModels.ChecklistTemplateLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
          limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const update = (
    id: string,
    officeId: string,
    data: ChecklistTemplatesModels.UpdateChecklistTemplateRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
