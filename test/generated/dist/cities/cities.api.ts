...
export namespace CitiesApi {
  export const paginate = (
    ...
    return AppRestClient.get({ resSchema: CitiesModels.CitiesPaginateResponseSchema }, `/cities`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(CitiesModels.CitiesPaginateOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(CitiesModels.CityPaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
  ...
  export const listCityLabels = (
    ...
    return AppRestClient.get({ resSchema: CitiesModels.ListCityLabelsResponseSchema }, `/cities/paginate/labels`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(CitiesModels.ListCityLabelsOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(CitiesModels.CityLabelFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
        limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const getCityLabelById = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: CitiesModels.CityLabelResponseDTOSchema }, `/cities/${id}/labels`, config);
  };
  export const findById = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: CitiesModels.CityResponseDTOSchema }, `/cities/${id}`, config);
  };
  ...
  export const archive = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.post({ resSchema: z.void() }, `/cities/${id}/archive`, undefined, config);
  };
  export const unarchive = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.post({ resSchema: z.void() }, `/cities/${id}/unarchive`, undefined, config);
  };
}
