...
export namespace ControlTowerCalendarQueries {
  ...
  export const keys = {
    ...
    ) => [...keys.all, "/calendar", from, to, type, search, poNumbers, containerNumbers, bookingNumbers] as const,
  };
   ...
   * @statusCodes [200, 401]
   */
  export const useGetCalendar = <TData>(
    {
      from,
      to,
      type,
    ...
    options?: AppQueryOptions<typeof ControlTowerCalendarApi.getCalendar, TData>,
    config?: AxiosRequestConfig,
  ) => {
    return useQuery({
      queryKey: keys.getCalendar(from, to, type, search, poNumbers, containerNumbers, bookingNumbers),
      queryFn: () =>
        ControlTowerCalendarApi.getCalendar(
          from,
          to,
          type,
          search,
