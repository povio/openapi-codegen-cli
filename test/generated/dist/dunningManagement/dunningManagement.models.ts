...
export namespace DunningManagementModels {
   ...
   */
  export const DunningPartnerDtoSchema = z
    .object({
      id: z.string(),
      name: z.string(),
      matchCode: z.string(),
   ...
   */
  export const DunningResponseDtoSchema = z
    .object({
      id: z.string(),
      partner: DunningPartnerDtoSchema,
      level: z.number(),
      dunningLevelId: z.string().nullish(),
      ...
      statusChangedOn: z.iso.datetime({ offset: true }),
      confirmedBy: DunningConfirmedByDtoSchema.nullish(),
   ...
   */
  export const DunningFilterDtoSchema = z
    .object({
      status: z.array(DunningStatusSchema).readonly(),
      partnerId: z.string().describe("Partner ID to filter by"),
      level: z.array(z.array(z.any()).readonly()).readonly().describe("Dunning level(s) to filter by"),
      outstandingAmountMin: z.number().describe("Minimum outstanding amount"),
      outstandingAmountMax: z.number().describe("Maximum outstanding amount"),
      createdFrom: z.iso.datetime({ offset: true }).describe("Created from (ISO date string)"),
      createdTo: z.iso.datetime({ offset: true }).describe("Created to (ISO date string)"),
      confirmedBy: z.string().describe("Confirmed by employee ID"),
    })
    .readonly();
   ...
   */
  export const DunningPdfConfigDTOSchema = z
    .object({
   ...
   */
  export const DunningPdfBusinessPartnerDTOSchema = z
    .object({ id: z.string(), name: z.string(), address: z.string(), referenceNumber: z.string().nullish() })
    .readonly();
   ...
   */
  export const DunningPdfInvoiceDTOSchema = z
    .object({
   ...
   * @property { string } currencyNotation
   */
  export const DunningPdfTotalDTOSchema = z
    .object({ invoicesSum: z.number(), dunningFee: z.number(), interest: z.number(), currencyNotation: z.string() })
    .readonly();
   ...
   */
  export const DunningPdfBankAccountDTOSchema = z
    .object({ displayValue: z.string(), iban: z.string(), bankName: z.string(), swiftBic: z.string() })
    .readonly();
   ...
   * @property { string } currencyNotation
   */
  export const DunningPdfUpcomingInvoiceDTOSchema = z
    .object({
      number: z.string(),
      issuingDate: z.iso.datetime({ offset: true }).nullish(),
      dueDate: z.iso.datetime({ offset: true }).nullish(),
      dueDays: z.number().nullish(),
   ...
   */
  export const DunningPdfPayloadDTOSchema = z
    .object({
      dunningId: z.string(),
      dunningLevel: z.number(),
      ...
      invoices: z.array(DunningPdfInvoiceDTOSchema).readonly(),
      total: DunningPdfTotalDTOSchema,
      bankAccount: DunningPdfBankAccountDTOSchema,
      employeeName: z.string().nullable(),
      config: DunningPdfConfigDTOSchema,
      language: CommonModels.LanguageEnumSchema.nullish(),
      upcomingInvoices: z.array(DunningPdfUpcomingInvoiceDTOSchema).readonly().nullish(),
    })
    .readonly();
   ...
   */
  export const CreateDunningWithInvoicesRequestDTOSchema = z
    .object({
      invoiceIds: z.array(z.string()).readonly().describe("Array of outstanding invoice IDs to include in the dunning"),
      dunningLevelId: z.string().describe("The ID of the dunning level to use for this dunning"),
      dunningFee: z
        .number()
        .gte(0)
        .describe("Optional dunning fee to use. If not provided, the fee from the dunning level will be used")
        .nullish(),
      interestRate: z.number().gte(0).lte(100).nullish(),
      includeUpcomingInvoices: z.boolean().nullish(),
    })
    .readonly();
   ...
   * @type { enum }
   */
  export const ListDunningsOrderParamEnumSchema = z.enum([
    "createdAt",
    "level",
    "outstandingAmount",
    "statusChangedOn",
   ...
   */
  export const ListDunningsResponseSchema = z.object({
    ...CommonModels.PaginationDtoSchema.shape,
    ...z.object({ items: z.array(DunningResponseDtoSchema).readonly() }).readonly().shape,
  });
