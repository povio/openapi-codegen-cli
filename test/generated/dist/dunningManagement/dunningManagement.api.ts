...
export namespace DunningManagementApi {
      ...
      `/offices/${officeId}/dunnings`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(DunningManagementModels.ListDunningsOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(DunningManagementModels.DunningFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
  ...
  export const createDunningWithInvoices = (
    partnerId: string,
    officeId: string,
    data: DunningManagementModels.CreateDunningWithInvoicesRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
  ...
  export const getDunningEml = (officeId: string, dunningId: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: z.instanceof(Blob) }, `/offices/${officeId}/dunnings/${dunningId}/eml`, {
      ...config,
      headers: {
