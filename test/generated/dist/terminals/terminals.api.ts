...
export namespace TerminalsApi {
  ...
  export const paginate = (
    ...
    return AppRestClient.get({ resSchema: TerminalsModels.TerminalsPaginateResponseSchema }, `/terminals`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(TerminalsModels.TerminalsPaginateOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(TerminalsModels.TerminalPaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
  ...
  export const paginateLabels = (
    ...
    return AppRestClient.get(
      ...
      `/terminals/paginate/labels`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(TerminalsModels.TerminalsPaginateLabelsOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(TerminalsModels.TerminalLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
          limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const getById = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: TerminalsModels.TerminalResponseDTOSchema }, `/terminals/${id}`, config);
  };
  ...
  export const archive = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.post({ resSchema: z.void() }, `/terminals/${id}/archive`, undefined, config);
  };
  export const unarchive = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.post({ resSchema: z.void() }, `/terminals/${id}/unarchive`, undefined, config);
  };
}
