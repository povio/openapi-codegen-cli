...
export namespace PortsApi {
  export const paginate = (
    ...
    return AppRestClient.get({ resSchema: PortsModels.PortsPaginateResponseSchema }, `/ports`, {
      ...config,
      params: {
        order: ZodExtended.parse(ZodExtended.sortExp(PortsModels.PortsPaginateOrderParamEnumSchema).optional(), order, {
          type: "query",
          name: "order",
        }),
        filter: ZodExtended.parse(PortsModels.PortPaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
  ...
  export const paginateLabels = (
    ...
    return AppRestClient.get({ resSchema: PortsModels.PortsPaginateLabelsResponseSchema }, `/ports/labels/paginate`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(PortsModels.PortsPaginateLabelsOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(PortsModels.PortLabelFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
        limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const findById = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: PortsModels.PortResponseDTOSchema }, `/ports/${id}`, config);
  };
}
