...
export namespace FactoringExportQueries {
  ...
  export const keys = {
    ...
    getBatch: (batchId: string, officeId: string) =>
      [...keys.all, "/offices/:officeId/factoring-exports/:batchId", batchId, officeId] as const,
  };
  ...
  export const useCreate = (
    options?: AppMutationOptions<
      typeof FactoringExportApi.create,
      { officeId: string; data: FactoringExportModels.CreateFactoringExportRequestDto }
    > &
  ...
  export const useGetBatch = <TData>(
    { batchId, officeId }: { batchId: string; officeId: string },
    options?: AppQueryOptions<typeof FactoringExportApi.getBatch, TData>,
    config?: AxiosRequestConfig,
  ) => {
    const { checkAcl } = useAclCheck();
