...
export const useGetHouseAwbData = <TData>({ positionId, hawbId, officeId }: { positionId: string, hawbId: string, officeId: string }, options?: AppQueryOptions<typeof WorkingDocumentsHouseAwbApi.getHouseAwbData, TData>) => {
  ...
  return useQuery({
    ...
    return WorkingDocumentsHouseAwbApi.getHouseAwbData(positionId, hawbId, officeId) },
    ...options,
    onError: options?.onError ?? queryConfig.onError,
  });
};
 ...
 * @statusCodes [201, 401]
 */
export const useGenerateHouseAwb = (options?: AppMutationOptions<typeof WorkingDocumentsHouseAwbApi.generateHouseAwb, { positionId: string, hawbId: string, officeId: string, data: CommonModels.GenerateWorkingDocumentRequestDto }> & MutationEffectsOptions) => {
  const queryConfig = OpenApiQueryConfig.useConfig();
  const { checkAcl } = useAclCheck();
  const { runMutationEffects } = useMutationEffects({ currentModule: moduleName });
