...
export const getListRoutesQueryOptions = ({ officeId, positionId }: { officeId: string, positionId: string }) => ({
...
export const useListRoutes = <TData>({ officeId, positionId }: { officeId: string, positionId: string }, options?: AppQueryOptions<typeof PositionRoutesApi.listRoutes, TData>) => {
  ...
  return useQuery({
    ...getListRoutesQueryOptions({ officeId, positionId }),
    queryFn: async () => {
    checkAcl(PositionRoutesAcl.canUseListRoutes({ officeId } ));
      return getListRoutesQueryOptions({ officeId, positionId }).queryFn();
    },
    ...options,
  });
};

export const prefetchListRoutes = (queryClient: QueryClient, { officeId, positionId }: { officeId: string, positionId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getListRoutesQueryOptions({ officeId, positionId }), ...options });
};
