...
export const getGetQueryOptions = ({ officeId, quoteId }: { officeId: string, quoteId: string }) => ({
...
export const useGet = <TData>({ officeId, quoteId }: { officeId: string, quoteId: string }, options?: AppQueryOptions<typeof QuoteDocumentApi.get, TData>) => {
  ...
  return useQuery({
    ...getGetQueryOptions({ officeId, quoteId }),
    queryFn: async () => {
    checkAcl(QuoteDocumentAcl.canUseGet({ officeId } ));
      return getGetQueryOptions({ officeId, quoteId }).queryFn();
    },
    ...options,
  });
};
...
export const prefetchGet = (queryClient: QueryClient, { officeId, quoteId }: { officeId: string, quoteId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getGetQueryOptions({ officeId, quoteId }), ...options });
};
