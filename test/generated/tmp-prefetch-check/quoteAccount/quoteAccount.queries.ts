...
export const getGetQueryOptions = ({ quoteId, officeId }: { quoteId: string, officeId: string }) => ({
...
export const useGet = <TData>({ quoteId, officeId }: { quoteId: string, officeId: string }, options?: AppQueryOptions<typeof QuoteAccountApi.get, TData>) => {
  ...
  return useQuery({
    ...getGetQueryOptions({ quoteId, officeId }),
    queryFn: async () => {
    checkAcl(QuoteAccountAcl.canUseGet({ officeId } ));
      return getGetQueryOptions({ quoteId, officeId }).queryFn();
    },
    ...options,
  });
};

export const prefetchGet = (queryClient: QueryClient, { quoteId, officeId }: { quoteId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getGetQueryOptions({ quoteId, officeId }), ...options });
};
