...
export const getFindAllQueryOptions = ({ search, officeId }: { search?: string, officeId?: string }) => ({
...
export const useFindAll = <TData>({ search, officeId }: { search?: string, officeId?: string }, options?: AppQueryOptions<typeof BankAccountsApi.findAll, TData>) => {
  ...
  return useQuery({
    ...getFindAllQueryOptions({ search, officeId }),
    queryFn: async () => {
    checkAcl(BankAccountsAcl.canUseFindAll());
      return getFindAllQueryOptions({ search, officeId }).queryFn();
    },
    ...options,
  });
};
...
export const prefetchFindAll = (queryClient: QueryClient, { search, officeId }: { search?: string, officeId?: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getFindAllQueryOptions({ search, officeId }), ...options });
};

export const getPaginateLabelsQueryOptions = ({ limit, order, filter, page, cursor }: { limit: number, order?: string, filter?: BankAccountsModels.BankAccountFilterDto, page?: number, cursor?: string }) => ({
...
export const getPaginateLabelsInfiniteQueryOptions = ({ limit, order, filter, cursor }: { limit: number, order?: string, filter?: BankAccountsModels.BankAccountFilterDto, cursor?: string }) => ({
