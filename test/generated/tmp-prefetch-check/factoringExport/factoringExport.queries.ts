...
export const getGetBatchQueryOptions = ({ batchId, officeId }: { batchId: string, officeId: string }) => ({
...
export const useGetBatch = <TData>({ batchId, officeId }: { batchId: string, officeId: string }, options?: AppQueryOptions<typeof FactoringExportApi.getBatch, TData>) => {
  ...
  return useQuery({
    ...getGetBatchQueryOptions({ batchId, officeId }),
    queryFn: async () => {
    checkAcl(FactoringExportAcl.canUseGetBatch({ officeId } ));
      return getGetBatchQueryOptions({ batchId, officeId }).queryFn();
    },
    ...options,
  });
};
...
export const prefetchGetBatch = (queryClient: QueryClient, { batchId, officeId }: { batchId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getGetBatchQueryOptions({ batchId, officeId }), ...options });
};
