...
export const getGetImportStatusQueryOptions = ({ jobId, officeId }: { jobId: string, officeId: string }) => ({
...
export const useGetImportStatus = <TData>({ jobId, officeId }: { jobId: string, officeId: string }, options?: AppQueryOptions<typeof MasterDataImportApi.getImportStatus, TData>) => {
  ...
  return useQuery({
    ...getGetImportStatusQueryOptions({ jobId, officeId }),
    queryFn: async () => {
    checkAcl(MasterDataImportAcl.canUseGetImportStatus({ officeId } ));
      return getGetImportStatusQueryOptions({ jobId, officeId }).queryFn();
    },
    ...options,
  });
};

export const prefetchGetImportStatus = (queryClient: QueryClient, { jobId, officeId }: { jobId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getGetImportStatusQueryOptions({ jobId, officeId }), ...options });
};
