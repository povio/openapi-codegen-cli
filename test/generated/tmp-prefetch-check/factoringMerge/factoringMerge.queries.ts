...
export const getGetMergeBatchQueryOptions = ({ officeId, batchId }: { officeId: string, batchId: string }) => ({
...
export const useGetMergeBatch = <TData>({ officeId, batchId }: { officeId: string, batchId: string }, options?: AppQueryOptions<typeof FactoringMergeApi.getMergeBatch, TData>) => {
  ...
  return useQuery({
    ...getGetMergeBatchQueryOptions({ officeId, batchId }),
    queryFn: async () => {
    checkAcl(FactoringMergeAcl.canUseGetMergeBatch({ officeId } ));
      return getGetMergeBatchQueryOptions({ officeId, batchId }).queryFn();
    },
    ...options,
  });
};

export const prefetchGetMergeBatch = (queryClient: QueryClient, { officeId, batchId }: { officeId: string, batchId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getGetMergeBatchQueryOptions({ officeId, batchId }), ...options });
};
