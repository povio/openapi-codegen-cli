...
export const getListRoutesQueryOptions = ({ officeId, quoteId }: { officeId: string, quoteId: string }) => ({
...
export const useListRoutes = <TData>({ officeId, quoteId }: { officeId: string, quoteId: string }, options?: AppQueryOptions<typeof QuoteRoutesApi.listRoutes, TData>) => {
  ...
  return useQuery({
    ...getListRoutesQueryOptions({ officeId, quoteId }),
    queryFn: async () => {
    checkAcl(QuoteRoutesAcl.canUseListRoutes({ officeId } ));
      return getListRoutesQueryOptions({ officeId, quoteId }).queryFn();
    },
    ...options,
  });
};
...
export const prefetchListRoutes = (queryClient: QueryClient, { officeId, quoteId }: { officeId: string, quoteId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getListRoutesQueryOptions({ officeId, quoteId }), ...options });
};
