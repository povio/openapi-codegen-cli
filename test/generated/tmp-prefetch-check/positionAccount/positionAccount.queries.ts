...
export const getGetQueryOptions = ({ positionId, officeId }: { positionId: string, officeId: string }) => ({
...
export const useGet = <TData>({ positionId, officeId }: { positionId: string, officeId: string }, options?: AppQueryOptions<typeof PositionAccountApi.get, TData>) => {
  ...
  return useQuery({
    ...getGetQueryOptions({ positionId, officeId }),
    queryFn: async () => {
    checkAcl(PositionAccountAcl.canUseGet({ officeId } ));
      return getGetQueryOptions({ positionId, officeId }).queryFn();
    },
    ...options,
  });
};

export const prefetchGet = (queryClient: QueryClient, { positionId, officeId }: { positionId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getGetQueryOptions({ positionId, officeId }), ...options });
};
