...
export const getListQueryOptions = ({ officeId, positionId }: { officeId: string, positionId: string }) => ({
...
export const useList = <TData>({ officeId, positionId }: { officeId: string, positionId: string }, options?: AppQueryOptions<typeof PositionChecklistApi.list, TData>) => {
  ...
  return useQuery({
    ...getListQueryOptions({ officeId, positionId }),
    queryFn: async () => {
    checkAcl(PositionChecklistAcl.canUseList({ officeId } ));
      return getListQueryOptions({ officeId, positionId }).queryFn();
    },
    ...options,
  });
};

export const prefetchList = (queryClient: QueryClient, { officeId, positionId }: { officeId: string, positionId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getListQueryOptions({ officeId, positionId }), ...options });
};
