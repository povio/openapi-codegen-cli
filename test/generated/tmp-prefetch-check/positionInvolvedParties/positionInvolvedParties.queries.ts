...
export const getFindByPositionIdQueryOptions = ({ officeId, positionId }: { officeId: string, positionId: string }) => ({
...
export const useFindByPositionId = <TData>({ officeId, positionId }: { officeId: string, positionId: string }, options?: AppQueryOptions<typeof PositionInvolvedPartiesApi.findByPositionId, TData>) => {
  ...
  return useQuery({
    ...getFindByPositionIdQueryOptions({ officeId, positionId }),
    queryFn: async () => {
    checkAcl(PositionInvolvedPartiesAcl.canUseFindByPositionId({ officeId } ));
      return getFindByPositionIdQueryOptions({ officeId, positionId }).queryFn();
    },
    ...options,
  });
};
...
export const prefetchFindByPositionId = (queryClient: QueryClient, { officeId, positionId }: { officeId: string, positionId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getFindByPositionIdQueryOptions({ officeId, positionId }), ...options });
};
