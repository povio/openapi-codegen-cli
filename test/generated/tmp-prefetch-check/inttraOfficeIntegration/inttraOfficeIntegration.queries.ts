...
export const getGetQueryOptions = ({ officeId }: { officeId: string }) => ({
...
export const useGet = <TData>({ officeId }: { officeId: string }, options?: AppQueryOptions<typeof InttraOfficeIntegrationApi.get, TData>) => {
  ...
  return useQuery({
    ...getGetQueryOptions({ officeId }),
    queryFn: async () => {
    checkAcl(InttraOfficeIntegrationAcl.canUseGet({ officeId } ));
      return getGetQueryOptions({ officeId }).queryFn();
    },
    ...options,
  });
};

export const prefetchGet = (queryClient: QueryClient, { officeId }: { officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getGetQueryOptions({ officeId }), ...options });
};
