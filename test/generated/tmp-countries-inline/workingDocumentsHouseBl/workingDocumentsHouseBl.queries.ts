...
export const getHouseBlDataQueryOptions = ({ positionId, houseBlId, officeId }: { positionId: string, houseBlId: string, officeId: string }) => ({
  ...
  queryFn: () => getHouseBlData(positionId, houseBlId, officeId),
});
...
export const previewHouseBlQueryOptions = ({ positionId, houseBlId, officeId }: { positionId: string, houseBlId: string, officeId: string }) => ({
  ...
  queryFn: () => previewHouseBl(positionId, houseBlId, officeId),
});
...
export const previewHouseBlEmlQueryOptions = ({ positionId, houseBlId, officeId }: { positionId: string, houseBlId: string, officeId: string }) => ({
  ...
  queryFn: () => previewHouseBlEml(positionId, houseBlId, officeId),
});
 ...
 * @statusCodes [201, 401]
 */
export const useGenerateHouseBl = (options?: AppMutationOptions<typeof generateHouseBl, { positionId: string, houseBlId: string, officeId: string, data: WorkingDocumentsHouseBlModels.GenerateWorkingDocumentRequestDto }> & MutationEffectsOptions) => {
  const queryConfig = OpenApiQueryConfig.useConfig();
  const { checkAcl } = useAclCheck();
  const { runMutationEffects } = useMutationEffects<typeof QueryModule.WorkingDocumentsHouseBl>({ currentModule: moduleName });
 ...
 * @statusCodes [200, 401]
 */
export const useGenerateDocumentEml = (options?: AppMutationOptions<typeof generateDocumentEml, { positionId: string, houseBlId: string, officeId: string, data: WorkingDocumentsHouseBlModels.GenerateWorkingDocumentRequestDto }> & MutationEffectsOptions) => {
  const queryConfig = OpenApiQueryConfig.useConfig();
  const { checkAcl } = useAclCheck();
  const { runMutationEffects } = useMutationEffects<typeof QueryModule.WorkingDocumentsHouseBl>({ currentModule: moduleName });
