...
export const getQueryOptions = ({ quoteId, officeId }: { quoteId: string, officeId: string }) => ({
  queryKey: keys.get(quoteId, officeId),
  queryFn: () => get(quoteId, officeId),
});
...
export const prefetchGet = (queryClient: QueryClient, { quoteId, officeId }: { quoteId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getQueryOptions({ quoteId, officeId }), ...options });
};
