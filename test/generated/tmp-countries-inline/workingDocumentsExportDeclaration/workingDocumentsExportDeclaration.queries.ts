...
export const getExportDeclarationDataQueryOptions = ({ officeId, positionId, exportDeclarationId }: { officeId: string, positionId: string, exportDeclarationId: string }) => ({
  ...
  queryFn: () => getExportDeclarationData(officeId, positionId, exportDeclarationId),
});
...
export const prefetchGetExportDeclarationData = (queryClient: QueryClient, { officeId, positionId, exportDeclarationId }: { officeId: string, positionId: string, exportDeclarationId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getExportDeclarationDataQueryOptions({ officeId, positionId, exportDeclarationId }), ...options });
};
...
export const previewExportDeclarationQueryOptions = ({ officeId, positionId, exportDeclarationId }: { officeId: string, positionId: string, exportDeclarationId: string }) => ({
  ...
  queryFn: () => previewExportDeclaration(officeId, positionId, exportDeclarationId),
});
...
export const prefetchPreviewExportDeclaration = (queryClient: QueryClient, { officeId, positionId, exportDeclarationId }: { officeId: string, positionId: string, exportDeclarationId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...previewExportDeclarationQueryOptions({ officeId, positionId, exportDeclarationId }), ...options });
};
 ...
 * @statusCodes [201, 401]
 */
export const useGenerateExportDeclaration = (options?: AppMutationOptions<typeof generateExportDeclaration, { officeId: string, positionId: string, exportDeclarationId: string, data: WorkingDocumentsExportDeclarationModels.GenerateWorkingDocumentRequestDto }> & MutationEffectsOptions) => {
  const queryConfig = OpenApiQueryConfig.useConfig();
  const { checkAcl } = useAclCheck();
  const { runMutationEffects } = useMutationEffects<typeof QueryModule.WorkingDocumentsExportDeclaration>({ currentModule: moduleName });
