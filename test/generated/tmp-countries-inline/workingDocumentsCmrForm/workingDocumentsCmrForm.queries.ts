...
export const getCmrDataQueryOptions = ({ positionId, cmrId, officeId }: { positionId: string, cmrId: string, officeId: string }) => ({
  ...
  queryFn: () => getCmrData(positionId, cmrId, officeId),
});
...
export const prefetchGetCmrData = (queryClient: QueryClient, { positionId, cmrId, officeId }: { positionId: string, cmrId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getCmrDataQueryOptions({ positionId, cmrId, officeId }), ...options });
};
...
export const previewCmrQueryOptions = ({ positionId, cmrId, officeId }: { positionId: string, cmrId: string, officeId: string }) => ({
  ...
  queryFn: () => previewCmr(positionId, cmrId, officeId),
});
...
export const prefetchPreviewCmr = (queryClient: QueryClient, { positionId, cmrId, officeId }: { positionId: string, cmrId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...previewCmrQueryOptions({ positionId, cmrId, officeId }), ...options });
};
 ...
 * @statusCodes [201, 401]
 */
export const useGenerateCmr = (options?: AppMutationOptions<typeof generateCmr, { positionId: string, cmrId: string, officeId: string, data: WorkingDocumentsCmrFormModels.GenerateWorkingDocumentRequestDto }> & MutationEffectsOptions) => {
  const queryConfig = OpenApiQueryConfig.useConfig();
  const { checkAcl } = useAclCheck();
  const { runMutationEffects } = useMutationEffects<typeof QueryModule.WorkingDocumentsCmrForm>({ currentModule: moduleName });
