...
export const getImportStatusQueryOptions = ({ jobId, officeId }: { jobId: string, officeId: string }) => ({
  queryKey: keys.getImportStatus(jobId, officeId),
  queryFn: () => getImportStatus(jobId, officeId),
});
...
export const prefetchGetImportStatus = (queryClient: QueryClient, { jobId, officeId }: { jobId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getImportStatusQueryOptions({ jobId, officeId }), ...options });
};
