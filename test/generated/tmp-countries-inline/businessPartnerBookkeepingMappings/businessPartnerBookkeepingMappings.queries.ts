...
export const getBookkeepingMappingsQueryOptions = ({ officeId, businessPartnerId }: { officeId: string, businessPartnerId: string }) => ({
  queryKey: keys.getBookkeepingMappings(officeId, businessPartnerId),
  queryFn: () => getBookkeepingMappings(officeId, businessPartnerId),
});
...
export const prefetchGetBookkeepingMappings = (queryClient: QueryClient, { officeId, businessPartnerId }: { officeId: string, businessPartnerId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">) => {
  return queryClient.prefetchQuery({ ...getBookkeepingMappingsQueryOptions({ officeId, businessPartnerId }), ...options });
};
