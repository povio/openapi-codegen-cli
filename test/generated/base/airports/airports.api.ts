...
export namespace AirportsApi {
  export const paginate = (
    ...
    return AppRestClient.get({ resSchema: AirportsModels.AirportsPaginateResponseSchema }, `/airports`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(AirportsModels.AirportsPaginateOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(AirportsModels.AirportPaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
  ...
  export const paginateLabels = (
    ...
    return AppRestClient.get(
      ...
      `/airports/labels/paginate`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(AirportsModels.AirportsPaginateLabelsOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(AirportsModels.AirportLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
          limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const findById = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: AirportsModels.AirportResponseDTOSchema }, `/airports/${id}`, config);
  };
}
