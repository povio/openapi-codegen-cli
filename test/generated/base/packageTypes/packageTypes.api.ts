...
export namespace PackageTypesApi {
  export const paginate = (
    ...
    return AppRestClient.get({ resSchema: PackageTypesModels.PackageTypesPaginateResponseSchema }, `/package-types`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(PackageTypesModels.PackageTypesPaginateOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(PackageTypesModels.PackageTypePaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
  ...
  export const paginateLabels = (
    ...
    return AppRestClient.get(
      ...
      `/package-types/paginate/labels`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(PackageTypesModels.PackageTypesPaginateLabelsOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(PackageTypesModels.PackageTypeLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
          limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const update = (
    id: string,
    data: PackageTypesModels.UpdatePackageTypeRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
