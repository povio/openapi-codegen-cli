...
export namespace ChecklistItemsApi {
  export const create = (
    officeId: string,
    data: ChecklistItemsModels.CreateChecklistItemRequestDTO,
      ...
      `/offices/${officeId}/checklist-items`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(ChecklistItemsModels.ChecklistItemsPaginateOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(ChecklistItemsModels.ChecklistItemFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
  ...
  export const paginateLabels = (
    ...
    return AppRestClient.get(
      ...
      `/offices/${officeId}/checklist-items/labels/paginate`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(ChecklistItemsModels.ChecklistItemsPaginateLabelsOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(ChecklistItemsModels.ChecklistItemLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
          limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const update = (
    id: string,
    officeId: string,
    data: ChecklistItemsModels.UpdateChecklistItemRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
