...
export namespace FoldersApi {
  ...
  export const getContent = (
    ...
    return AppRestClient.get(
      ...
      `/offices/${officeId}/folders/${folderId}`,
      {
        ...config,
        params: {
          order: ZodExtended.parse(FoldersModels.GetContentOrderParamSchema.optional(), order, {
            type: "query",
            name: "order",
          }),
          filter: ZodExtended.parse(FoldersModels.FolderContentFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
  ...
  export const rename = (
    officeId: string,
    folderId: string,
    data: FoldersModels.RenameFolderRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
  ...
  export const deleteFolder = (officeId: string, folderId: string, config?: AxiosRequestConfig) => {
    return AppRestClient.delete({ resSchema: z.void() }, `/offices/${officeId}/folders/${folderId}`, undefined, config);
  };
