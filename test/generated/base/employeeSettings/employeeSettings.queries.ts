...
export namespace EmployeeSettingsQueries {
   ...
   * @statusCodes [200, 401]
   */
  export const useGetAll = <TData>(
    options?: AppQueryOptions<typeof EmployeeSettingsApi.getAll, TData>,
    config?: AxiosRequestConfig,
  ) => {
    return useQuery({
      queryKey: keys.getAll(),
      queryFn: () => EmployeeSettingsApi.getAll(config),
      ...options,
    });
  };
  ...
  export const useUpdate = (
    options?: AppMutationOptions<
      typeof EmployeeSettingsApi.update,
      { key: string; data: EmployeeSettingsModels.UpdateEmployeeSettingDto }
    > &
    ...
    return useMutation({
      mutationFn: ({ key, data }) => EmployeeSettingsApi.update(key, data, config),
      ...options,
