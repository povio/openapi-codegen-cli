...
export namespace CargoTypesApi {
  export const paginate = (
    ...
    return AppRestClient.get({ resSchema: CargoTypesModels.CargoTypesPaginateResponseSchema }, `/cargo-types`, {
      ...config,
      params: {
        order: ZodExtended.parse(
          ZodExtended.sortExp(CargoTypesModels.CargoTypesPaginateOrderParamEnumSchema).optional(),
          order,
          { type: "query", name: "order" },
        ),
        filter: ZodExtended.parse(CargoTypesModels.CargoTypePaginationFilterDtoSchema.optional(), filter, {
          type: "query",
          name: "filter",
        }),
  ...
  export const paginateLabels = (
    ...
    return AppRestClient.get(
      ...
      `/cargo-types/labels/paginate`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(CargoTypesModels.CargoTypesPaginateLabelsOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(CargoTypesModels.CargoTypeLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
          limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const findById = (id: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get({ resSchema: CargoTypesModels.CargoTypeResponseDTOSchema }, `/cargo-types/${id}`, config);
  };
