...
export namespace RemarkTemplatesApi {
      ...
      `/offices/${officeId}/remark-templates/paginate/labels`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(RemarkTemplatesModels.RemarkTemplatesPaginateLabelsOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(RemarkTemplatesModels.RemarkTemplateLabelFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
  ...
  export const list = (
    ...
    return AppRestClient.get(
      ...
      `/offices/${officeId}/remark-templates`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(RemarkTemplatesModels.RemarkTemplatesListOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(RemarkTemplatesModels.RemarkTemplateFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
          limit: ZodExtended.parse(z.number().gte(1).lte(100).describe("Items per response").default(20), limit, {
  ...
  export const create = (
    officeId: string,
    data: RemarkTemplatesModels.CreateRemarkTemplateRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
  ...
  export const update = (
    id: string,
    officeId: string,
    data: RemarkTemplatesModels.UpdateRemarkTemplateRequestDTO,
    config?: AxiosRequestConfig,
  ) => {
