...
export namespace WorkingDocumentsApi {
      ...
      `/offices/${officeId}/positions/${positionId}/working-documents`,
      {
        ...
        params: {
          order: ZodExtended.parse(
            ZodExtended.sortExp(WorkingDocumentsModels.WorkingDocumentsListOrderParamEnumSchema).optional(),
            order,
            { type: "query", name: "order" },
          ),
          filter: ZodExtended.parse(WorkingDocumentsModels.WorkingDocumentFilterDtoSchema.optional(), filter, {
            type: "query",
            name: "filter",
          }),
  ...
  export const findById = (positionId: string, id: string, officeId: string, config?: AxiosRequestConfig) => {
    return AppRestClient.get(
