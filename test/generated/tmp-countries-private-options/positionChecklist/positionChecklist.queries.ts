...
const listQueryOptions = ({ officeId, positionId }: { officeId: string, positionId: string }) => ({
...
export const prefetchList = (queryClient: QueryClient, { officeId, positionId }: { officeId: string, positionId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">): void => {
  void queryClient.prefetchQuery({ ...listQueryOptions({ officeId, positionId }), ...options });
};
