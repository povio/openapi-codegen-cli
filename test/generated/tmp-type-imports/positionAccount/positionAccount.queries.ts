...
const getQueryOptions = ({ positionId, officeId }: { positionId: string, officeId: string }) => ({
...
export const prefetchGet = (queryClient: QueryClient, { positionId, officeId }: { positionId: string, officeId: string }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">): void => {
  void queryClient.prefetchQuery({ ...getQueryOptions({ positionId, officeId }), ...options });
};
