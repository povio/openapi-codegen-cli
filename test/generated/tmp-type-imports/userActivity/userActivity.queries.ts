...
import { OpenApiQueryConfig, type AppQueryOptions } from "@povio/openapi-codegen-cli";
...
const getQueryOptions = ({ officeId, entityType, entityId, activeThresholdMinutes }: { officeId: string, entityType: string, entityId: string, activeThresholdMinutes?: number }) => ({
...
export const prefetchGet = (queryClient: QueryClient, { officeId, entityType, entityId, activeThresholdMinutes }: { officeId: string, entityType: string, entityId: string, activeThresholdMinutes?: number }, options?: Omit<Parameters<QueryClient["prefetchQuery"]>[0], "queryKey" | "queryFn">): void => {
  void queryClient.prefetchQuery({ ...getQueryOptions({ officeId, entityType, entityId, activeThresholdMinutes }), ...options });
};
